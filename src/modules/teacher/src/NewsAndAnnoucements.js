...
const NewsAndAnnouncements = () => {
    const { userData, fetchAnnouncements, deleteDocById } = useContext(UserContext);
        ...
        try {
            ...
            if (response?.success) {
                const announcementsWithFlag = response?.data?.map((announcement) => ({
                    ...announcement,
                    isMine: announcement.createdBy?.id === userData?.teacher_id,
                }));

                const now = new Date();

                const filteredAnnouncements = announcementsWithFlag.filter((announcement) => {
                    if (announcement.isMine) return true;

                    if (
                        announcement.isScheduled &&
                        new Date(announcement.scheduleDateTime) > now
                    ) {
                        return false;
                    }
                    ...
                    {announcements.map((announcement, index) => (
                        ...
                        >
                            ...
                            <div style={styles.contentSection}>
                                ...
                                {/* Schedule Info */}
                                {announcement.isScheduled && announcement?.isMine && (
