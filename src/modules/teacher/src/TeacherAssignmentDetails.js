...
const TeacherAssignmentDetails = () => {
    ...
    useEffect(() => {
        if (!sectionId || !assignmentId || sections?.length === 0) return;

        const getAssignmentsData = async () => {
            try {
                const response = await fetchAssignments(sectionId);
        ...
        getAssignmentsData();
    }, [sectionId, assignmentId, sections]);
    ...
    const getFilteredStudents = () => {
        if (!sectionData?.students || !assignmentData) return [];
    ...
    const LoadingScreen = () => (
        <div style={styles.loadingContainer}>
            <div style={styles.loadingContent}>
                ...
                <h3 style={styles.loadingTitle}>Loading Assignment Details</h3>
                <p style={styles.loadingText}>Please wait...</p>
            </div>
        </div>
    );

    if (!sectionData || !assignmentData) {
